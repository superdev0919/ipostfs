...
export const PinForm = (props) => {
    ...
    const [maxReplication, setMaxReplication] = useState(
        (
            props.values?.replication_factor_max ?? defaults.maxReplication
        ).toString()
    );
    const [minReplication, setMinReplication] = useState(
        (
            props.values?.replication_factor_min ?? defaults.minReplication
        ).toString()
    );
    const [shardSize, setShardSize] = useState(
        (props.values?.shard_size ?? defaults.shardSize).toString()
    );
